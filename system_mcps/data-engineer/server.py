...
def generate_synthetic_data_ddpm(
    ...
    """
    Generate synthetic data using Gaussian Multinomial Diffusion (GMD/DDPM) via tabular-gmd API endpoint.
    
    This function first tries to use the tabular-gmd API service (running in Docker) which provides
    optimized GPU-accelerated synthetic data generation. If the API is unreachable, it falls back
    to using the local tabular-gmd library with numpy backend.
    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "for categorical features. Provides high-quality synthetic data generation with better preservation "
                "of complex relationships and distributions compared to traditional GANs. Automatically detects and "
                "handles mixed data types (numerical and categorical). Supports CSV, JSON, and Parquet formats. "
                "Requires minimum 10 rows (50 recommended) for meaningful results. Training time varies based on "
                "data size and epochs (default: 10 epochs for MCP). Use this for research-grade synthetic data "
